...
if __name__ == "__main__":
    ...
    lora_layers = [i for i in range(12)]
    layer_names = [f'roberta.encoder.layer.{i}.attention.self.{att_weight}.weight' for i in lora_layers for att_weight in ['query', 'value']] 
    #chosen_state_dict = {k: v for k,v in model.state_dict() if k in layer_names} 
    ...
    for step, train_batch in enumerate(active_dataloader):
        ...
        results[f"batch-{step}"] = {}
        for layer, st_weights in model.state_dict().items():
            if layer in layer_names:
                weights = {layer: st_weights}
                weights2 = {"model." + n: p for n, p in weights.items()}  # the original model is wrapped as model2.model
                trace_estimate = trace_hessian_hutchinson(model2, train_batch, weights2, num_estimates=args.num_estimates,
                                                      default_eval=True, seed=args.seed)
                accelerator.log({f"batch {step}, layer {layer} Trace Estimate": trace_estimate},
                            step=args.num_estimates)
                print(f"done layer {layer} for batch {step}: {trace_estimate.item()}")
                results[f"batch-{step}"][layer] = trace_estimate.item()
