...
if __name__ == "__main__":
    ...
    print(f"num layers = {len(layer_names)}")
    ...
    for step, train_batch in enumerate(active_dataloader):
        ...
        for idx, layer in enumerate(layer_names):
            ...
            accelerator.log({f"batch {step}, layer {layer} ({idx}) Trace Estimate": trace_estimate},
                            step=args.num_estimates)
            print(f"done layer {idx} for batch {step}")
            results[f"batch-{step}"][f"{layer}-idx"] = trace_estimate.item()
