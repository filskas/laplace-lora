...
def choose_metric(task_name, output_dir, completed_steps, test_loader_name):
    if task_name is not None:
        if task_name in ['wnli', 'rte', 'mrpc', 'cola', 'sst2', 'qnli', 'qqp', 'mnli']:
            return evaluate.load("glue", task_name,
                                   experiment_id=f"{output_dir}_step_{completed_steps}_{test_loader_name}")
        elif task_name in ['cb', 'wic', 'boolq']:
            return evaluate.load("super_glue", task_name,
                                   experiment_id=f"{output_dir}_step_{completed_steps}_{test_loader_name}")
    else:
        return evaluate.load("accuracy")


def log_eval_and_train_metrics(model, args, test_loader_list, test_loader_names, is_regression, completed_steps, accelerator, eval_dataloader, epoch, train_eval_dataloader):
    for test_loader, test_loader_name in zip(test_loader_list, test_loader_names):

        metric = choose_metric(args.task_name, args.output_dir, completed_steps, test_loader_name)
        ...
        eval_train_metric = choose_metric(args.task_name, args.output_dir, completed_steps, train_loader_name)
        samples_seen = 0
        output_dicts = []
        ted = train_eval_dataloader
        for step, batch in enumerate(ted):
            with torch.no_grad():
                outputs = model(**batch)
            ...
            if accelerator.num_processes > 1:
                if step == len(ted) - 1:
                    predictions = predictions[: len(ted.dataset) - samples_seen]
                    references = references[: len(ted.dataset) - samples_seen]
                else:
                    samples_seen += references.shape[0]
        ...
        eval_train_metric_res = eval_train_metric.compute()
        all_results = {f"train_{k}": v for k, v in eval_train_metric_res.items()}
        if args.with_tracking:
            accelerator.log(all_results, step=epoch)
...
def main():
    ...
    # peft_config = LoraConfig(task_type="SEQ_CLS", inference_mode=False, r=args.lora_r, lora_alpha=0, lora_dropout=args.lora_dropout)
    ...
    print(f"optimizer params: {optimizer_grouped_parameters}")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        log_eval_and_train_metrics(model, args, test_loader_list, test_loader_names, is_regression, completed_steps, accelerator, eval_dataloader, epoch, train_eval_dataloader)
        ...
        if args.with_tracking and not finished:
            accelerator.log({"total_loss": total_loss}, step=epoch)
