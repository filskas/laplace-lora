...
def main():
    ...
    for name, param in model.named_parameters():
        #if 'original_module.out_proj' in name:
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, train_batch in enumerate(active_dataloader):
            ...
            if isinstance(checkpointing_steps, int):
                if (completed_steps+1) % checkpointing_steps == 0 or completed_steps == 0:
                    for test_loader, test_loader_name in zip(test_loader_list, test_loader_names):
                        ...
                        if args.with_tracking:
                            accelerator.log(all_results, step=completed_steps)
            ...
            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0 or completed_steps - 1 == 0:
                    if args.with_tracking:
                        accelerator.log({"total_loss": total_loss}, step=completed_steps - 1)

        # if args.with_tracking:
        #     accelerator.log({"total_loss": total_loss}, step=completed_steps)
